...
def _crawl_naver_keywords(text):
 ...
 #url = re.search(r'(https?://\S+)', text.split('|')[0]).group(0)
    if "음악순위" in text:
        ...
        for data in soup.find_all("span", class_="name"):
            ...
            for i in range(10):
                if len(keywords) >= 10 :
                    break
                keywords.append(str(i+1)+"위: "+ title[i] )

            # 한글 지원을 위해 앞에 unicode u를 붙혀준다.
            return u'\n'.join(keywords)      

    elif "농구랭킹" in text:
        url = 'https://sports.news.naver.com/basketball/index.nhn'
        req = urllib.request.Request(url)
        ...
        url = 'https://sports.news.naver.com/kfootball/index.nhn'
        req = urllib.request.Request(url)
